...
```{r}
# center the data
r.centered <- scale(b, center = TRUE, scale = FALSE)
...
The dimensionality of this variance-covariance matrix is 200 by 200. This is because...
...
lazlo_b <- laszlo[,,3]
lazlo_r.centered <- scale(lazlo_b, center = TRUE, scale = FALSE)
lazlo_var.total <- sum(apply(lazlo_r.centered, 2, function(x) sum(x^2))) / nrow(lazlo_r.centered)

print(var.total)
print(lazlo_var.total)
```

There is a higher total variance of the blue matrix in the Laszlo image than in the Lev image.
...
```{r, echo = TRUE, include = TRUE}
var.pcr.1/var.total # data is 66% 
...
pcr.2 <- r.centered %*% eigs$vectors[,2]
...
"If you compute the covariance of the first principal component with the second, you should expect to get a single number that represents the strength of the linear relationship between the two variables. A positive covariance indicates that the two variables tend to increase or decrease together, while a negative covariance indicates that the two variables tend to move in opposite directions. The magnitude of the covariance indicates the strength of the relationship, with larger magnitudes indicating stronger relationships."- GPT
...
This is because there is more variance in the lazlo image so its easier to distinguish him from the background. The Lev image has less variance so it requires more principal components to minially represent him.
